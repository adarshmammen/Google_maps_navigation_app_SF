...
void Graph::_import_from_file( const string& input_file_name )
{
	...
	///   These values are separated by 'white space'. 
	int start_vertex, end_vertex, line_num;
	double edge_weight;
	int vertex_id = 0;
	while(ifs >> line_num)//ifs>>start_vertex
	{
		ifs>>start_vertex;
		if (start_vertex == -1)
		{
		...
		ifs >> edge_weight;
		if (line_num <0){
			cout<<start_vertex<<" "<<end_vertex<<" "<<edge_weight<<endl;
		}

		///3.2.1 construct the vertices
		BaseVertex* start_vertex_pt = get_vertex(start_vertex +1);
		BaseVertex* end_vertex_pt = get_vertex(end_vertex+1);
		...
		///3.2.3 update the fan-in variables
...
int Graph::get_edge_code( const BaseVertex* start_vertex_pt, const BaseVertex* end_vertex_pt ) const
{
	...
	int a = min(start_vertex_pt->getID(),end_vertex_pt->getID());
	int b = max(start_vertex_pt->getID(),end_vertex_pt->getID());

	return a*m_nVertexNum + b;
...
void Graph::get_adjacent_vertices( BaseVertex* vertex, set<BaseVertex*>& vertex_set )
{
	...
	{
		set<BaseVertex*>* vertex_pt_set = get_vertex_set_pt(vertex, m_mpFaninVertices);//Fanout
